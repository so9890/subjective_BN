...
\begin{figure}[h]
	...
	\node[circll] (A) at (-4,4) {Production};
	\node[circll] (B) at (7,4) {Household};
	\node[sphere] (C) at (-4,-5) {Research};
	\node[circll] (D) at (7,-5) {Government};
	\node[dialog] (F) at (3.5,-3.0) {Moralischer \\ Modus};
	\node[draw=none] (G) at (3.5,-11) {};
	
	\draw [->] (A) to [out=0,in=90] (F);
	\draw [->] (B) to [out=180,in=90] (F);
	\draw [->] (F) to node[]{4} (D);
	...
	\draw [->] (A) to [out=-140, in=-150, out looseness=1.5, in looseness=3, inner sep=0pt,outer sep=0pt] node[pos=0.2, outer sep=2.5, swap]{1} (F);
	\draw [->] (B) to [out=-40, in=-30, out looseness=1.5, in looseness=3, inner sep=0pt,outer sep=0pt] node[pos=0.2, outer sep=2.5]{7} (C) ;
	\end{pgfinterruptboundingbox}
	
	\draw [->] (B) to node[pos=0.75]{5} (D);
	\draw [->] (B) to node[pos=0.75]{6} (F);
	\end{tikzpicture}
