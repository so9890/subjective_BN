...
\begin{frame}{Motivation}
	\begin{itemize}[<+-| alert@+>]
		\setbeamercolor{alerted text}{} %change the font color
		\setbeamerfont{alerted text}{}
		\item How to best implement emission targets in line with climate goals? % Frage
		\vspace{3mm}
		\item Problem:
			\begin{itemize}
				...
				\item[-] A \textbf{carbon tax} lowers the use fossil energy but also reduces research in established industries %state of the art
			\end{itemize} 
				\vspace{3mm}
		\item I discuss how \textbf{labor income taxes} may be used to maintain some fossil research today while meeting emission targets
				\vspace{3mm}
...
\begin{frame}{This paper}
	...
	\begin{itemize}
		...
		\item<+-> An \alert{emission limit} constrains the government: positive net emissions before 2050; net-zero emissions starting from 2050
	\end{itemize}
	\pause
	\begin{center}
		\begin{figure}
...
\begin{itemize}[<+->]
			...
			\\  \normalsize{\alert{\ar precludes studying combinations of compositional and scaling policies}}
			...
			% I argue that we should study optimal env. policies allowing for level adjustments of production! I provide am example scenario where adjusting the level becomes part of the optimal policy
				\vspace{2mm}
				\item \alert{This paper}: targeting the scale of production is optimal due to (i) directed technical change and (ii) lack of research subsidies \footnotesize{\citep{Acemoglu2012TheChange}}
	...
	\begin{itemize}
		\item[-]  fiscal policy generally passive \footnotesize{ \citep{ LansBovenberg1994EnvironmentalTaxation, Goulder1995EnvironmentalGuide, Barrage2019OptimalPolicy}} % Not an env. policy instrument
		\item[-] arise as environmental policy tool due to \textbf{income inequality} \footnotesize{\citep{Jacobs2019RedistributionCurves, Dobkowitz2022, Douenne2022OptimalHouseholds}}
		\item[-] I add a novel motive for the use of labor income taxes within the optimal environmental policy
	\end{itemize}
...
\begin{itemize}
	...
	\end{itemize}
	\vspace{0mm}
	\item<+-> \textbf{equal} distribution of  CO$_2$ emissions among countries %\footnotesize{\citep{RobiouDuPont2017EquitableGoals}}
\end{itemize}
\vspace{-2mm}
\pause
\begin{center}
	...
	\begin{center}
		...
		\resizebox{5.6in}{!}{
			\begin{tabular}{l|lll}
				...
				${{\eta}}$  &0.79 & \small{returns to  research}& \cite{Fried2018ClimateAnalysis}\\%\rdelim\}{3}{5cm}[\normalfont\makecell{\cite{Fried2018ClimateAnalysis}}] \\
				${{\phi}}$  &0.75& \small{cross-sector knowledge spillovers}& \cite{Barbieri2021GreenPolicy} \\
				$({\varepsilon_y, \varepsilon_e})$&(0.05, 1.50)&price elasticity of substitution& \cite{Fried2018ClimateAnalysis}\\	
...
\begin{frame}{Conclusion}
	\begin{itemize}[<+-| alert@+>]
		...
		\begin{itemize}
			\item[-] Outlook: role for labor income tax remains since carbon tax revenues are not redistributed lump-sum \ar labor market distortion
