...
\begin{frame}{Contribution to the literature}
	\begin{itemize}[<+->]
		...
		\begin{itemize}
			\item[-]  generally passive \footnotesize{ \citep{ LansBovenberg1994EnvironmentalTaxation, Goulder1995EnvironmentalGuide, Barrage2019OptimalPolicy}} % Not an env. policy instrument
		...
		\vspace{2mm}
		\item \alert{This paper}: use of labor income tax is optimal due to  directed technical change % and (ii) lack of research subsidies
...
\begin{frame}{Model}
	\begin{figure}[h]
		...
		\begin{tikzpicture}[auto,scale=.7, transform shape]
			...
			\draw [->] (B22) to node[pos=0.45, swap]{\alert{$\pmb{\tau_{\iota}} \uparrow$:  labor supply $\downarrow$ \ar emissions $\downarrow$}} (D2);
			%	\draw [->] (B22) to node[pos=0.3, swap]{\alert{$\pmb{\tau_{\iota}}<0$:    labor supply $\uparrow$ \ar output $\uparrow$}} (D2);
			\draw [->] (D1) to node[pos=0.5, swap]{Transfers} (B3);
...
\begin{frame}{Model}
	\begin{figure}[h]
		...
		\begin{tikzpicture}[auto,scale=.7, transform shape]
			...
			\draw [->] (B22) to node[pos=0.65, swap]{\alert{Tax on labor, $\pmb{\tau_{\iota}}$:}} (D2);
			\draw [->] (B22) to node[pos=0.45, swap]{\alert{$\pmb{\tau_{\iota}} \uparrow$:  labor supply $\downarrow$ \ar emissions $\downarrow$}} (D2);
			\draw [->] (B22) to node[pos=0.3, swap]{\alert{$\pmb{\tau_{\iota}} \downarrow$:    labor supply $\uparrow$ \ar output $\uparrow$}} (D2);
			\draw [->] (D1) to node[pos=0.5, swap]{Transfers} (B3);
		...
		\begin{center}
			...
			\resizebox{5.6in}{!}{
				\begin{tabular}{l|lll}
					...
					$({\varepsilon_y, \varepsilon_e})$&(0.05, 1.50)&\small{price elasticity of substitution}&\small \cite{Fried2018ClimateAnalysis}\\	
					%	($\alpha_F$, $\alpha_G$, $\alpha_N$) &(0.72, 0.91, 0.36)&\\
					$\frac{A_{F0}}{A_{G0}}$&8.27&\small{relative productivity} &\small energy and fossil shares \citep{EIAEnergy} \\
...
\begin{frame}{Integrating labor income taxes into the environmental policy}
	...
	\begin{figure}[h!!]
		...
		\begin{subfigure}{0.45\textwidth}		
			\caption{{Average marginal income tax, $\tau_{\iota t}$}}
			%	\captionsetup{width=.45\linewidth}
			%	\includegraphics[width=1\textwidth]{../codding_model/own_basedOnFried/optimalPol_010922_revision/figures/all_13Sept22/NewCalib_polTaulFixed_T_dTaulAvS_Sun2_emnet1_spillover0_knspil3_xgr0_nsk0_sep0_extern0_PV1_etaa0.79_lgd1.png}
			\includegraphics[width=1\textwidth]{../codding_model/own_basedOnFried/optimalPol_010922_revision/figures/all_13Sept22/NewCalib_pol_TvsNoT_dTaulAvS_base_emnet1_Sun2_spillover0_knspil3_xgr0_nsk0_sep0_extern0_PV1_etaa0.79_lgd1.png}
		\end{subfigure}
...
\begin{frame}{Effect of integrated policy}
	...
	\begin{block}{}
		\begin{itemize}
			\item Adjustment in carbon tax directs research activity.
			\item Changes in labor income tax adjusts level of production. %side effects on labor market
		\end{itemize}
	\end{block}	
\end{frame}
...
\begin{frame}{Sensitivity to knowledge spillovers}
	...
	\begin{figure}[h!!]
		...
		\begin{subfigure}{0.42\textwidth}		
			\caption{ Deviation average marginal income tax in \%}
			%	\captionsetup{width=.45\linewidth}
