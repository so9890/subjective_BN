...
\begin{frame}{This paper}
	...
	\begin{center}
		\begin{figure}
			\centering
			\textbf{\textcolor{blue}{Set in relation to current emissions}US net CO$_2$ emission limit in Gt}\\
...
\begin{frame}{Model overview}
	\begin{figure}[h]
		...
		\begin{tikzpicture}[auto,scale=.7, transform shape]
			...
			\draw [->] (B1) to node[pos=0.75, swap]{Workers and scientists} (BA1);
			\draw [->] (BA2) to node[pos=0.75, swap]{Final good} (B2);
   			\draw [->] (A4) to node[pos=0.5, swap]{Tax on carbon, $\pmb{\tau_F}$}   (D4);
		\end{tikzpicture}
...
\begin{frame}{Carbon tax: 1. Effect on fossil demand}
	\begin{figure}[h]
		\vspace{-4mm}
		\centering
		\begin{tikzpicture}[auto,scale=.7, transform shape]
...
\begin{frame}{Machine producers: Research}
	\tr{Explain in picture using previous graph}
	\begin{itemize}
		\item machine producers maximize profits
		\item monopolistically competitive sector: set price taking demand for machines into account
		\item demand for machines rises with their productivity, $A_{Jt}$
		\item machine producers invest in productivity of their machines by employing scientists
	\end{itemize}
...
\begin{frame}{Carbon tax: Effect on Research}
	\begin{figure}[h]
		\vspace{0mm}
		\centering
		\begin{tikzpicture}[auto,scale=.6, transform shape]
%			\node[circll] (A) at (0,16) {\textbf{Final}\textbf{ Good}}; 
%			\node[circll] (B) at (-6,14) {\textbf{Energy}};
%			\node[circll] (C) at (5,14) {\textbf{{Non-energy}}};
	...
	\end{figure}
\begin{itemize}
	\item free movement of scientists \ar in equilibrium:
	 \begin{align*}
...
\begin{frame}{Carbon tax: Effect on Research}
	\begin{figure}[h]
		\vspace{0mm}
		\centering
		\begin{tikzpicture}[auto,scale=.4, transform shape]
			%			\node[circll] (A) at (0,16) {\textbf{Final}\textbf{ Good}}; 
			%			\node[circll] (B) at (-6,14) {\textbf{Energy}};
			%			\node[circll] (C) at (5,14) {\textbf{{Non-energy}}};
			\node[circll] (D) at (-10,10) {\textbf{Machines }\\\textbf{Fossil}};
	...
	\begin{itemize}
		...
		\end{align*}
	\item[\ar] $s_F\downarrow$ and $s_G \uparrow$
	\end{itemize}
\end{frame}
...
\item higher carbon tax to foster green research under net-zero limit;\\ labor subsidy stabilizes output % the emission limit is too strict to maintain some fossil research bcs it comes with a higher fossil to green energy ratio. It is then optimal to at least internalize green dynamic spillovers
\end{itemize}
\end{frame}
