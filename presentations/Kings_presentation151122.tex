...
\begin{frame}{2nd: Effect of a carbon tax on \alert{research activity}}
	...
	\pause
		\begin{itemize}
			\item \alert{carbon tax lowers returns to research in fossil sector \ar scientists shift from fossil to green sector}
		\end{itemize}
...
\begin{frame}{Government trade-off}
	...
	\begin{itemize}[<+-| alert@+>]
		...
		\begin{enumerate}
			\item[a)] lower emissions; in long run only green production is feasible
			\item[b)] keep productivity high
		\end{enumerate}
	\vspace{3mm}
	\item Effect carbon tax
	\begin{enumerate}
		\item[a)] reduces emissions by lowering fossil demand
		\item[b)] directs research \ar affects productivity
		\begin{itemize}
			\item[-] if want to foster green research
			\ar higher carbon tax \ar but reduces returns to labor %\ar costly in terms of output % reduces share of fossil energy 
...
\begin{frame}{Productivity of research and inefficiencies}
...
\normalsize
	\begin{enumerate}
		...
		\end{itemize}
%		\item decreasing returns to research, $\eta<1$
		\item \alert{knowledge spillovers:} knowledge from other sectors increases research productivity \pause
		\begin{itemize}
