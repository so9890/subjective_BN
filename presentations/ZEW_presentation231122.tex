...
%\institute{University of Bonn} 
\date{\small{ZEW, Mannheim\\ November 23, 2022 }} 
%\subject{} 
\begin{document}
...
\begin{frame}{Contribution}
	...
	%	With knowledge spillovers \ar (i) increasing policy intervention, (ii) potentially advantageous to maintain some fossil research
		...
		\item Quantitative framework builds on \cite{Fried2018ClimateAnalysis}. I add: %\ar new qualitative insights
		\begin{itemize}
			\item[-] \alert{dynamic optimal policy analysis}
			\item[-] \alert{dynamic emission target} eventually declining to \alert{net-zero emissions}
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{align*}
	...
	& (2)\ A_{Jit}=A_{Jit}(s_{jit})%A_{Jt-1}\left(1+\gamma\left(\frac{s_{Jit}}{\rho_J}\right)^\eta\left(\frac{A_{t-1}}{A_{Jt-1}}\right)^\phi\right)
	%	\tikzmarkend{sixth}
\end{align*}
...
\begin{frame}{Innovation}
	...
	\large
	\begin{align*}
		A_{Jit}(s_{jit})=\alert{A_{Jt-1}}\left(1+\gamma\left(\frac{s_{Jit}}{\rho_J}\right)^\eta\left(\frac{A_{t-1}}{A_{Jt-1}}\right)^\phi\right)
	\end{align*}
	\normalsize
	\begin{enumerate}
...
\begin{minipage}[t!]{0.43\textwidth}
	\vspace{0mm}
	\begin{itemize}
		\item[] $A_{Jt}$: sector-specific technology level
		\vspace{-7mm}		
		\item[] $A_t$: aggregate technology level
		\vspace{-2mm}
...
\begin{frame}{Innovation}
	\large
	\begin{align*}
		A_{Jit}(s_{jit})={A_{Jt-1}}\left(1+\gamma\alert{\left(\frac{s_{Jit}}{\rho_J}\right)^{\eta}}{\left(\frac{A_{t-1}}{A_{Jt-1}}\right)^\phi}\right)
	\end{align*}
	\normalsize
	\begin{enumerate}
...
\begin{frame}{Innovation}
	\large
	\begin{align*}
		A_{Jit}(s_{jit})={A_{Jt-1}}\left(1+\gamma\left(\frac{s_{Jit}}{\rho_J}\right)^\eta\alert{\left(\frac{A_{t-1}}{A_{Jt-1}}\right)^\phi}\right)
	\end{align*}
	\normalsize
	\begin{enumerate}
		\item path dependency: sector-specific knowledge renders scientists more productive
		%				\begin{itemize}
			...
			%		\end{itemize}
		\item decreasing returns to research, $\eta<1$
		\item \alert{knowledge spillovers} from other sectors increases productivity of scientists in sector $J$
	\end{enumerate}
\small
\vspace{4mm}
\hspace{-2mm}
...
\begin{minipage}[t!]{0.4\textwidth}
	...
	\begin{itemize}
		...
		\vspace{-2mm}	
		\item[] $\omega$\ : emissions per unit of fossil \vspace{-2mm}
		\item[] $\delta$\ \ : carbon sinks (constant)
	\end{itemize}
\end{minipage}
		...
		\begin{center}
			...
			%		\label{tab:calib}
				...
				\begin{tabular}{l|lll}
					...
					${{\phi}}$  &0.50& \small{knowledge spillovers}&  \\
...
\vspace{-4mm}
\hfill
\hyperlink{altems}{\tiny{$\rightarrow$ alternative emission limit,}}\hyperlink{mec0}{\tiny{$\rightarrow$ mechanism}}
\end{frame}
...
\begin{frame}{Sensitivity of optimal policy to knowledge spillovers}
	...
	\vspace{-5mm}
	\hfill
	\hyperlink{conc}{\tiny{$\rightarrow$ back}}\textbf{}
\end{frame}
...
\begin{frame}{Sensitivity of optimal policy to other parameters}
	...
	\vspace{-4mm}
	\hfill
	\hyperlink{backOPT}{\tiny{$\rightarrow$ back,}}\hyperlink{conc}{\tiny{$\rightarrow$ back conclusion}}\textbf{}
\end{frame}
\end{document}