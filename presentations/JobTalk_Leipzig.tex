...
%\institute{University of Bonn} 
\date{\small{Presentation at Leipzig University\\ Feburary 6, 2023 }} 
%\subject{} 
\begin{document}
...
{ % these braces make the change local to the single frame
	\setbeamertemplate{footline}{ \footnotesize{\ \ Summary accessible to Master Students}\hfill \scriptsize{\insertframenumber} \hspace{0.5mm}\  \vspace{3.2mm}}
...
\begin{frame}{Model}
	\begin{figure}[h]
		...
		\begin{tikzpicture}[auto,scale=.7, transform shape]
			...
			\draw [->] (B22) to node[pos=0.45, swap]{\textcolor{black!1}{$\pmb{\tau_{\iota}} \uparrow$:  labor supply $\downarrow$ \ar emissions $\downarrow$}} (D2);
			\draw [->] (B22) to node[pos=0.3, swap]{\textcolor{black!1}{$\pmb{\tau_{\iota}} \downarrow$:    labor supply $\uparrow$ \ar output $\uparrow$}} (D2);
...
\begin{frame}{Innovation}
	...
	\begin{enumerate}
		...
		\item {cross-sectoral knowledge spillovers} 
				\item[-] \alert{inefficiencies: one-period patents}%: knowledge from other sectors increases productivity of scientists in sector $J$} %\footnotesize{\citep{Barbieri2021GreenPolicy}}
	\end{enumerate}
	\small
	\vspace{4mm}
	\hspace{-2mm}
