...
\begin{frame}
	...
	\begin{tikzpicture}[auto,scale=.7, transform shape]
		...
		%	  \node[] (A) at (-10,3) {Projects: \ \ \ \ \ \ \ \ \ };	
		\node[elli] (F) at (-6.6,2.8) {{\hyperlink{prodmod}{Meeting Climate Targets:}}\\ {The Optimal}\\ {Fiscal Policy Mix}};
		
		\node[elli] (G) at (-3,0) {{Labor Income Taxes,} \\ {Social Responsibility, and Inequality}};
		\node[elli] (H) at (-0.5,2.8) {Voluntary  Reduction, Green Skills,\\ and the Environment};
		\node[elli] (I) at (6.2,2.8) {{Green Demand,}\\ Clean Innovation, and Lobbying};
