...
\begin{itemize}[<+-| alert@+>]
	...
	\vspace{3mm}
	\item and natural scientists suggest stricter emission limits to meet temperature targets  \citep[e.g. IPCC reports ][]{Rogelj2018MitigationDevelopment., IPCC2022}.
	\vspace{3mm}
	...
	\vspace{3mm}
\item \textbf{Is there a role for fiscal policies in optimally meeting emission limits?}
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\vspace{3mm}
	\item<+->  an \alert{emission limit constraints} the government when  {maximising social welfare}
	\vspace{3mm}
	\item<+-> the government can use \alert{corrective} and \alert{distortionary income taxes}
	\vspace{3mm}
	%	\item<+-> profitability of research in each sector determines sector growth
	\item<+-> the representative household \alert{elastically} supplies \alert{low- and high-skilled labour }
	\vspace{2mm}
	\begin{itemize}
		\item[-]<+-> \textit{reductive effect} of income taxes
		\vspace{1mm}
		\item[-]<+-> \textit{recomposing effect} income tax: green sector is skill-biased  \citep{Consoli2016DoCapital}
	\end{itemize}
...
\begin{enumerate}
	...
	\vspace{5mm}
	\item<+-> income taxes contribute to social welfare by 0.11\%   %but are nnot necessary to meet the emission limit %
...
\vspace{4mm}
\\
s.t.\ C_t=z_h \lambda_t (w_{ht}h_{ht})^{\alert{\pmb{1-\tau_{lt}}}}+(1-z_h) \lambda_t (w_{lt}h_{lt})^{\alert{\pmb{1-\tau_{lt}}}}%+Gov_t
\\
...
\begin{frame}{Model: Production}
	...
	\begin{minipage}[t!]{1\textwidth}
		\begin{align*}
		\tikzmarkin{third}(7.8,2.8)(-2.8,-2.3)
		\underset{\{x_{ijt}\}_{i=0}^1, L_{jt}}{\max}\ & \pi_{jt}=(1-\alert{\pmb{\tau_{jt}}})p_{jt}J_t-w_{ljt}L_{jt}-\int_{0}^{1}p_{ijt}x_{ijt}di\\ 
		s.t.\ & J_{t}=L_{jt}^{1-\alpha_j}\int_{0}^{1}A^{1-\alpha_j}_{ijt}x_{ijt}^{\alpha_j}di \\
		\ & \tau_{nt}=\tau_{gt}=0
		\tikzmarkend{third}
		\end{align*}
...
&\max_{\{\tau_{ft}\}_{t=0}^{T}, \{\tau_{lt}\}_{t=0}^{T}} \sum_{t=0}^{T}\beta^t U_{t}\\
s.t. \hspace{4mm}
&(1)\ \ T_t= \text{income tax}(\lambda_t, \tau_{lt})+\tau_{ft}p_{ft}F_{t}\\
& (2)\ \text{behaviour of firms, scientists, and households}\\
& (3)\ \text{feasibility} \\
&\alert{
...
\begin{frame}{Conclusion}
	...
	\item emissions are mainly targeted by the corrective tax
		\vspace{3mm}
	\item the welfare gains of an income tax amount to 0.11\%  over the horizon from 2020 to 2080 %\small{(and to 0.85\% with one skill type)}
   \vspace{3mm}
...
%	\begin{block}{}
%While the corrective tax is set to meet emission targets and recomposes production,
