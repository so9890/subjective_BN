...
\begin{minipage}[t!]{1\textwidth}
	\begin{align*}
	\tikzmarkin{third}(3.6,1.9)(-6.4,-1.6)
J_{t}=L_{jt}^{1-\alpha_j}\int_{0}^{1}A^{1-\alpha_j}_{ijt}x_{ijt}^{\alpha_j}di \\  \pi_{ft}=p_{ft}(1-\alert{\pmb{\tau_{ft}}})F_t-w_{lft}L_{ft}-\int_{0}^{1}p_{fit}x_{fit}di %=x_{ft}^{\alpha_f}\left(A_{ft}L_{ft}\right)^{1-\alpha_f} \\
