...
\paragraph{In case I want to add capital taxation} 
...
\paragraph{An environmental perspective}
 Adding environmental externalities to the classical public finance framework may change the light with which efficiency costs (of either capital or labour taxes) are perceived. Instead of merely reducing welfare, direct benefits through output reduction arise by lowering the externality. 

In the presence of corrective tax instruments which direct production to a non-polluting alternative, however, distortionary labour taxes are not employed as environmental policy instruments under standard preferences.\footnote{ Compare my paper on Sustainable production and demand. } Nevertheless, some (non-economist) scholars argue for the necessity to reduce current consumption levels in developed economies in the light of threats to planetary boundaries. This can be understood by the pressing time frame to correct for climate externalities in particular, threats to other planetary boundaries (\textit{what are the distinct features due to Dasgupta?}), the irreversibility of damage done (such as tipping points of the climate and biodiversity). Furthermore, the argument is put forward that no perfectly clean technology can never be innovated.  
In sum, the need to reduce environmental externalities rather quickly and the potential non-existence of perfectly green technologies could render classical public-finance tax instruments environmental policy tools.

\paragraph{Trade-offs}
Counteracting indirect effects could lower the direct effectiveness of reduction policies to lower the externality. Proponents of a reduction policy especially focus on consumption by the rich which might suffer less from a reduction.\footnote{\ Abstracting from this aspect might make a reduction in consumption-poor household's income which, in the data, have a higher marginal propensity to consume polluting goods. \textit{Could be an important aspect in the model}.} Targeting these households in particular for environmental reasons, in contrast, will lower high skill labour supply and investment into it. Yet, these skills are especially used in green sectors of the economy \citep{Consoli2016DoCapital}. 

However, whether a reduction policy is indeed optimal for environmental reasons has never been studied in a general equilibrium model. 

\paragraph{The quantitative exercise}
\begin{enumerate}
...
\item[-] focus analysis on effects on externality (welfare measure)
\item sensitivity: look at different utility specifications (a) habits/social preferences, (b) non-homothetic preferences, income-dependent marginal propensities to consume polluting goods (use first paper's preferences), (c) social profit of leisure
\end{enumerate}
