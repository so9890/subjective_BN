...
\tr{add why lump-sum trans might not be feasible (Fried/Barrage)}
 %I demonstrate that if labor supply, in addition, is elastic, hours worked are inefficiently high. % even though the environmental tax is set to implement the efficient share of dirty production.
%Lump-sum transfers serve to reduce labor supply through an income effect.
% As a result, 
...
The Ramsey planner maximizes social welfare as in the core model, but is constrained by exogenous emission limits. The planner sets the environmental tax and the progressivity parameter of the non-linear income tax scheme. Environmental tax revenues are redistributed through the income tax scheme. The context replicates the scenario analyzed in the first part of the paper where lump-sum transfers were not available but the efficient allocation was feasible by redistributing through the income tax scheme.
