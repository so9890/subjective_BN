...
It is established in the literature that absent any other distortion, an environmental tax equal to the social cost of the externality implements the efficient allocation. I argue that this result crucially depends on the use of lump-sum transfers to redistribute environmental tax revenues; otherwise work effort is inefficiently high. %I demonstrate that if labor supply, in addition, is elastic, hours worked are inefficiently high. % even though the environmental tax is set to implement the efficient share of dirty production.
%Lump-sum transfers serve to reduce labor supply through an income effect.
% As a result, 
 When no lump-sum transfers are feasible,  environmental taxes are optimally complemented with progressive labor income taxes. % to lower inefficiently high hours worked. 
