...
%	\includepdf[pages=-]{../titlepage.pdf}
	...
	\begin{abstract}
		\begin{singlespacing}
			\textbf{Abstract \ }
			Some scholars call for reductive policies to handle environmental limits. I show that, indeed,  
			the efficient allocation consists of both a recomposing and a reductive element. As a result, when no lump-sum transfers are available, distortionary income taxes are a necessary complement to the Pigouvian tax to implement the efficient allocation.
			The finding has important consequences for, first, the discussion of the optimal usage of carbon tax revenues. Second, when environmental taxes are not lump-sum redistributed, the optimal environmental policy increases equity. Third, the result complements the \textit{weak} double-dividend hypothesis: there exists a lower bound on distortionary income taxes after which a further reduction decreases efficiency.
...
%\input{app_impo}
%\input{appendix_fisc}<- Important notes on nature etc
