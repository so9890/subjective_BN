...
\begin{align}
\underset{s, H}{\max}\ & U(C,H; F)\\ s.t\ \ & C=Y-Gov\\ & \text{behavior of firms and household}.
\end{align}

The optimality conditions differ from the social planner's ones in through the derivatives on government revenues, $Gov$:
\begin{align}
wrt.\ s:\hspace{4mm} & U_C\left(\frac{dY}{dF}\frac{dF}{ds}+\frac{dY}{dG}\frac{dG}{ds}-\frac{dGov}{ds}\right)=-U_F\frac{dF}{ds}, \label{eq:sbs}
\\
wrt.\ H:\hspace{4mm} & U_C\left(\frac{dY}{dH}-\frac{dGov}{dH}\right)+U_F\frac{dF}{dH}=-U_H\label{eq:sbh}. 
\end{align}
...
\begin{prop}\label{prop:no_red}
	If lump-sum transfers are not available and environmental tax revenues are consumed by the government, the efficient allocation is infeasible. Income taxes are optimally used to lower inefficiently high hours worked. The Pigou principle is violated. \tr{See Table \ref{tab:lin_nolst}.}
\end{prop}
