...
I define the continuation value as the potential utility over the infinite horizon starting from the last explicit maximization period when labor and research inputs are the same as in the last period. 
\begin{align}
PV=\sum_{s=T+1}^{\infty} \beta^{s}u(C_s,h_{hT}, h_{lT}, s_{gT}, s_{nT}, s_{fT}).
\end{align}

I make three simplifying assumptions to substitute the infinite sum in the optimization problem. First, I assume that under all future governments, government consumption is zero, which implies that $C_s=(1-s_s)Y_s$, where $s_s$ is the savings rate in period s, that is, the share of output which is spent on machines. Second, I assume that the savings rate is the same as in the last optimization period, $T$. Then, consumption grows at the same rate as output. 
