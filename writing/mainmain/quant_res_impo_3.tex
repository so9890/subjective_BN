...
In figure \ref{fig:optAll_percLf_dyn}, I contrast the optimal allocation under the benchmark regime with income tax scheme, black solid graph, with the optimal allocation without labor income tax, the blue dashed graph. As a benchmark to the optimal policy, the figure depicts the social planner's allocation\footnote{\tr{I formulate the social planner's problem in appendix section xxx. }} by the orange dotted graph. The efficient allocation can be perceived as the allocation the Ramsey planner seeks to implement. However, she may not be able to achieve the efficient allocation due to the reliance on tax instruments.
All graphs depict percentage changes relative to the laissez-faire allocation of the same period.\footnote{\ Figure \ref{fig:LF} in section \ref{app:quant_res} shows the laissez-faire allocation. } Except for panel (f) which compares the environmental tax in the model with and without income tax. 
...
This becomes apparent when only feeding the optimal income tax into the model and comparing the resulting allocation to the laissez-faire economy. This is done for the results in figure \ref{fig:LF_vs_onlytaul} in the appendix. The green-to-fossil energy mix only reduces slightly.
...
The reductive effect of the progressive income tax a higher fossil share does not conflict with meeting the emission limit. 
