...
\subsubsection{Relation of recomposition and production function}
\paragraph{If the aggregate production function features  decreasing returns to scale in labor, the efficient adjustment of the dirty labor share is a reduction}
\begin{proof}
	...
	Under decreasing returns to scale it holds that the left-hand side is decreasing in $L_F$ and the right-hand side is decreasing in $L_G$. Hence, the adjustment to satisfy equation \ref{eq:mpl_eff} when an externality is present implies a decrease in $L_F$ and/or a rise in $L_G$.
...
\subsubsection{Derivation $\tau_f^*$ without lump-sum transfers}
...
\end{align}
Where under decreasing returns to scale the second summand is negative and the first is positive. \tr{To be continued.} 
...
\begin{align}
\tau_\iota= -\frac{H}{w}\frp{\left(\frp{Y}{L_G}\right)}{L_G}.=\left(\left(\frp{G}{L_G}\right)^2\frac{\partial ^2Y}{\partial G^2}+\frp{Y}{G}\frac{\partial ^2G}{\partial L_G ^2}\right).
\end{align}
