...
The reason is that the green energy production features a low labor share, while fossil is more prone to labor supply. Similarly, the non-energy sector is characterized by a high labor share. As a result, the reduction in labor supply affects the fossil sector more than the green one, making fossil production relatively more expensive; consider panels (h) and (i). Furthermore, the non-energy good becomes more expensive. Therefore, the reduction in fossil production is more intense than in green energy production. 
Hence, in a model without skill heterogeneity and endogenous growth, a progressive labor tax has an advantageous effect on the energy mix but also increases the energy share. In sum, net emissions decrease; see panel (n).
...
In figure \ref{fig:LF_vs_onlytaul_xgr} I show the effect of the optimal income tax set in the model with exogenous growth in the exogenous growth model. 

In comparison to the former model, there is now a recomposing effect stemming from skill heterogeneity. The high-to-low skill ratio in equilibrium reduces and increases when labor tax progressivity is reduced in 2050; panel (f). There are, thus, two counteracting mechanisms affecting the composition of final output. While the 
capital-labor share mechanism discussed earlier makes fossil energy more costly, the recomposition of skills makes green production and energy production overall more costly. 
...
I now add endogenous growth but abstract from skill heterogeneity in the simplified model considered first. 
Figure \ref{fig:LF_vs_onlytaul_nsk} shows the allocation in a model with one skill type where the optimal labor income tax is implemented in comparison to the laissez-faire allocation. This exercise sheds light on the effect of labor income taxation absent a recomposing effect through skill supply. 
In contrast to the model without endogenous growth, the capital-labor channel seems not to be active. Instead,  endogenous growth offsets this channel. The higher price in the fossil sector directs research to the fossil sector, while the smaller input market reduces profits from fossil research. In equilibrium, 
...
\end{minipage}
	...
	\end{minipage}
	\begin{minipage}[]{0.32\textwidth}
		\centering{\footnotesize{(d)  Green-to-fossil science}}
		%	\captionsetup{width=.45\linewidth}
	...
	\end{minipage}
	\begin{minipage}[]{0.32\textwidth}
		\centering{\footnotesize{(e)  Output}}
		%	\captionsetup{width=.45\linewidth}
	...
	\end{minipage}
	\begin{minipage}[]{0.32\textwidth}
		\centering{\footnotesize{(f)  Consumption}}
		%	\captionsetup{width=.45\linewidth}
	...
	%\end{minipage}
	%\begin{minipage}[]{0.32\textwidth}
	%	\centering{\footnotesize{(h)  Energy-to-output ratio}}
	%	%	\captionsetup{width=.45\linewidth}
	%	\includegraphics[width=1\textwidth]{../../codding_model/own_basedOnFried/optimalPol_190722_tidiedUp/figures/all_10Aout22/CountTaul_modnsk_target_EY_spillover0_sep1_extern0_PV1_etaa0.79_lgd1.png}
	%\end{minipage}
	\begin{minipage}[]{0.32\textwidth}
		\centering{\footnotesize{(h) Wage fossil scientists}}
		%	\captionsetup{width=.45\linewidth}
	...
	%\end{minipage}
	\begin{minipage}[]{0.32\textwidth}
		\centering{\footnotesize{(i) Price fossil energy}}
		%	\captionsetup{width=.45\linewidth}
...
There is  a more sizeable recomposition in labor towards the fossil sector; consider panel (c) than seems to be reflected in the output ratio.

Indeed, 
the recomposing effect on research is negligible. Only the amount of scientists employed in the fossil sector reduces minimally (panel (d)); the effect on green and non-energy research is not visible (panels (e) and (f)). 
