...
\subsubsection{Proof proposition \ref{prop:1}: Absent lump-sum transfers, hours are inefficiently high}\label{app:nolumpsum_hourshigh}
\begin{proof}\textit{Absent lump-sum transfers, hours are inefficiently high when the environmental tax implements efficient share of dirty production and the wage rate is decreasing in equilibrium hours worked.}
	...
	Under the assumption that the wage rate is decreasing or constant in equilibrium hours, it holds that the right-hand side is below or equal unity since $H^*\leq H_{FB}$. Thus,
	\begin{align}
...
\end{align}
If returns to scale of clean and aggregate production are decreasing or constant and decreasing for at least one, than $\tau_\iota >0$ and the optimal income tax rate is progressive. 
...
\subsection{Model with  functional forms}
