...
In this section, I discuss the effect of the optimal income tax on certain variables by comparing a scenario where only the optimal income tax is implemented to the laissez-faire allocation. First, I focus on the model without skill heterogeneity to abstract from the recomposing effect. Second, I consider results in the full model.


Figure \ref{fig:LF_vs_onlytaul_nsk} shows the allocation in a model with one skill type where the optimal labor income tax is implemented in comparison to the laissez-faire allocation. This exercise sheds light on the effect of labor income taxation absent a recomposing effect through skill supply. Since overall production shrinks, one might expect a reduction in research effort, too.
...
In figure \ref{fig:LF_vs_onlytaul}, I display the effect of the labor income tax in the full model when the environmental tax is set to zero. The recomposing effect on research is negligible. Only the amount of scientists employed in the fossil sector reduces minimally; the effect on green and non-energy research is not visible. 
The recomposing effect on green-to-fossil production seems to be driven mainly by a recomposition in labor towards the fossil sector; consider panel (e). The share of energy in total output declines minimally as the non-energy labor good, indeed, becomes relatively more abundant but energy and non-energy being complements. Hence, the recomposing effect of labor income taxes on emissions seems not to be amplified much by endogenous growth. 

The adjustment to the progressive income tax is mainly driven by a decline in wages for scientists. The wage rate paid to fossil scientists reduces by less than the wage rate paid to green scientists (panels (g) and (h)). While returns to research decrease as labor supply overall reduces - as discussed in the model without skill heterogeneity above, the reduction in high-skill labor is more intense, hence, the reduction in the wage rate for green research is more pronounced.
...
\begin{figure}[h!!]
	...
	\end{minipage}
%\begin{minipage}[]{0.32\textwidth}
%	\centering{\footnotesize{(b) Green to Fossil research\\ \  }}
%	%	\captionsetup{width=.45\linewidth}
%	\includegraphics[width=1\textwidth]{../../codding_model/own_basedOnFried/optimalPol_190722_tidiedUp/figures/all_10Aout22/CountTaul_modtest_target_sgsff_spillover0_sep1_extern0_PV1_etaa0.79_lgd0.png}
%\end{minipage}
	\begin{minipage}[]{0.32\textwidth}
		\centering{\footnotesize{(c) Green research\\ \ }}
		%	\captionsetup{width=.45\linewidth}
	...
	\end{minipage}
\begin{minipage}[]{0.32\textwidth}
	\centering{\footnotesize{(e)  Green-to-fossil labor}}
	%	\captionsetup{width=.45\linewidth}
