...
The household derives utility from consumption, $C$, but experiences disutility from hours worked, $H$. An externality from fossil production, $F$, decreases household utility. It is taken as given by the household.
...
The negative externality of fossil production makes it efficient to alter the share of fossil labor so that  a marginal reallocation of labor to the fossil sector would raise output.\footnote{\ Note that $U_F<0$ by assumption so that the right-hand side is positive and that $\frac{dG}{ds}<0$. Hence, the marginal product of labor in the fossil sector is higher compared to the green sector in the efficient allocation.} %Hence,$\frp{Y}{F}\frp{F}{s}>-\frp{Y}{G}\frp{G}{s}$ is efficient. }
...
The reason is, that when the carbon tax is set to implement the efficient share of dirty labor, the wage rate reflects the marginal costs of hours by raising emissions. I show in Appendix \ref{app:incometax0} that the wage rate can be written as:

\begin{align*}
w = \frp{Y}{H}+\frac{U_F}{U_C}\frp{F}{H}.
\end{align*}
Therefore, households internalize the externality of  hours worked in their labor supply decision. Since $U_F<0$, the wage rate and, thus, labor supply decline. Proposition \ref{prop:1} condenses this result.

\begin{prop}\label{prop:1}
	The efficient allocation is implemented by an environmental tax and lump-sum transfers. There is no role for distortive labor income taxation, $\tau_{\iota}^*=0$. When the environmental tax implements the efficient share of fossil labor, the wage rate fully captures the marginal effect of hours worked on the externality. 
\end{prop}
