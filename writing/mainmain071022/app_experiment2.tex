...
%\end{minipage}
\floatfoot{Notes:{ Graphs show the percentage deviation from the business-as-usual policy induced by a carbon tax equal to US\$185. Dashed graphs refer to the model without income tax; solid graphs refer to the model with labor income tax, $\tau_{\iota}=0.181$.  }}
\end{figure}
...
\end{minipage}
\begin{minipage}[]{0.32\textwidth}
\centering{\footnotesize{(c) Green energy }}
%	\captionsetup{width=.45\linewidth}
...
\end{minipage}
\begin{minipage}[]{0.32\textwidth}
	\centering{\footnotesize{(d) Energy share to GDP}}
	%	\captionsetup{width=.45\linewidth}
