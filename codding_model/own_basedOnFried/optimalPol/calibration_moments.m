...
%shareProdFossil =  (pOil.*prodPetrol + pCoal.*prodCoal + pGas.*prodGas).*1e9./(gdp*1e9);
GDP= gdp; % normalise to one!
shareConsumeFossil =  (pOil.*cOil + pCoal.*cCoal + pGas.*cGas).*1e9./(gdp*1e9); 
shareConsumeEnergy = pE*E./gdp;
% 
