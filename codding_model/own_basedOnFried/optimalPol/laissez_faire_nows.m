...
%- transform variables directly instead of in code
 ...
 sff    = S/(1+exp(x(list.choice=='sff')));
 sg     = S/(1+exp(x(list.choice=='sg')));
 sn     = S/(1+exp(x(list.choice=='sn')));
...
q=q+1;
f(q)= phis+gammasf - (gammaa*etaa*(A_lag./Af_lag).^phii.*sff.^(etaa-1).*pf.*(1-tauf).*F.*(1-alphaf).*Af_lag)./(rhof^etaa.*Af); 
%8
q=q+1;
