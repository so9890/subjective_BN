function f=laissez_faire_nows_sepSe(x, params, list, pol, laggs, indic, Emlim,t)
% Model
...
se      = upbarH/(1+exp(x(list.choice=='se')));
...
% q=q+1;
% f(q)= (chiis)*se^sigmaas-(muu*wsf-gammasf); % scientist hours supply
q=q+1;
f(q)= (chiis)*se^sigmaas-((muu*wsg-gammasg));
q=q+1;
...
q=q+1;
f(q)= gammasg.*(upbarH-se);
q=q+1;
f(q)= gammasn.*(upbarH-sn);
%market for scientists
 q=q+1;
 f(q)= sff+sg-se; % energy scientists move freely
