function  [ceq]= calibRem_nows_fsolve(x, MOM, list, trProd, paramss, poll, Af, An, Ag)
% this function backs out missing functions:
% 

read_in_pars_calib

% calibration to 2019 (lag = 2010-2014)
Af_lag  = exp(x(list.calib3=='Af_lag'));
Ag_lag  = exp(x(list.calib3=='Ag_lag'));
An_lag  = exp(x(list.calib3=='An_lag'));
sff     = exp(x(list.calib3=='sff'));
sg      = exp(x(list.calib3=='sg'));
sn      = exp(x(list.calib3=='sn'));
wsg      = exp(x(list.calib3=='wsg'));
wsf      = exp(x(list.calib3=='wsf'));

wsn      = exp(x(list.calib3=='wsn'));
%  1    = (exp(x(list.calib3=='1'))+LB_1);
chiis   = exp(x(list.calib3=='chiis'));
gammaa  = exp(x(list.calib3=='gammaa'));
 phii    =0.5;% 1/(1+exp(x(list.calib3=='phii')));

% sigmaas = exp(x(list.calib3=='sigmaas'));
% chiis   = exp(x(list.calib3=='chiis'));
% gammaa  = exp(x(list.calib3=='gammaa'));
% rhon  = exp(x(list.calib3=='rhon'));
% rhog  = exp(x(list.calib3=='rhog'));
% rhof  = exp(x(list.calib3=='rhof'));


% auxiliary
% rhog=rhon/MOM.rhong;
% rhof=rhon/MOM.rhonf;
A =  (rhof*Af+rhon*An+rhog*Ag)/(rhof+rhon+rhog);
A_lag  = (rhof*Af_lag+rhon*An_lag+rhog*Ag_lag)/(rhof+rhon+rhog);

% required vars from previous calibration steps
[ C, ~, ~, ~, pf, FF, pn, pg, ~, ~, ~, N, G,...
    ~, ~, ~, ~, ~, ~, ~, ~]=resProd_GE(list, trProd, MOM , paramss, poll, 'calib'); 

% gammaa = MOM.growth/((sn/rhon)^1*(A_lag/An_lag)^phii); % n is growth rate in non-energy technology: n0=An'/An-1
%  q=q+1;
% ceq(q)= MOM.S-(ws/(chiis.*C.^th1)).^(1/sigmaas); % hours supply by scientists to find sigmaas
% chiis= ws/(C^th1*MOM.S^sigmaas);

%% target equations


q=0;

% % target gammaa
 % q=q+1;
  % ceq(q)= (A/A_lag-1)-MOM.growth; % targeting 5 year growth rate
%   q=q+1;
%   ceq(q)= gammaa - MOM.growth/((sn/rhon)^1*(A_lag/An_lag)^phii);

% ceq(q)=
 q=q+1;
 ceq(q)= gammaa - MOM.growth/((sn/rhon)^1*(A_lag/An_lag)^phii);

% LOM => lagged technology
q=q+1;
ceq(q) = Af- Af_lag*(1+gammaa*(sff/rhof)^1*(A_lag/Af_lag)^phii);
q=q+1;
ceq(q) = Ag- Ag_lag*(1+gammaa*(sg/rhog)^1*(A_lag/Ag_lag)^phii);
% q=q+1;
%  ceq(q) = An- An_lag*(1+gammaa*(sn/rhon)^1*(A_lag/An_lag)^phii);
%  scientist demand
q=q+1;
ceq(q)= wsf - (gammaa*1*(A_lag./Af_lag).^phii.*sff.^(1-1).*pf.*(1-tauf).*FF.*(1-alphaf).*Af_lag)./(rhof^1.*Af); 
%8
q=q+1;
ceq(q)= wsg - (gammaa*1*(A_lag./Ag_lag).^phii.*sg.^(1-1).*pg.*G.*(1-alphag).*Ag_lag)./(rhog^1.*(1-taus)*Ag);
%9
q=q+1;
ceq(q)= wsn - (gammaa*1*(A_lag./An_lag).^phii.*sn.^(1-1).*pn.*N.*(1-alphan).*An_lag)./(rhon^1.*An);

q=q+1;
ceq(q)= sg+sff+sn-MOM.targethour;
%  q=q+1;
%  ceq(q)= sn-MOM.targethour;
%  q=q+1;
%  ceq(q)= sff-MOM.targethour;
q=q+1;
ceq(q)= sg-(wsg/(chiis)).^(1/sigmaas);
q=q+1;
ceq(q)= sff-(wsf/(chiis)).^(1/sigmaas);  % equal disutility as for other labour => pins down ws
q=q+1;
ceq(q)= sn-(wsn/(chiis)).^(1/sigmaas); 
%  q=q+1;
%  ceq(q)= MOM.rhon-rhon;  % equal disutility as for other labour => pins down ws

% fprintf('number equations %d, number variables %d', q, length(x));
end