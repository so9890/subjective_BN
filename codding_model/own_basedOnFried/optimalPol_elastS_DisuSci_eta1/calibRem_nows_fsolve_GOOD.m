function  [ceq]= calibRem_nows_fsolve_GOOD(x, MOM, list, trProd, paramss, poll, Af, An, Ag)
...
%  ceq(q)= (A/A_lag-1)-MOM.growth; % targeting 5 year growth rate
 q=q+1;
 ceq(q)= gammaa - MOM.growth/((sn/rhon)*(A_lag/An_lag)^phii);
...
q=q+1;
ceq(q) = Af- Af_lag*(1+gammaa*(sff/rhof)*(A_lag/Af_lag)^phii);
q=q+1;
ceq(q) = Ag- Ag_lag*(1+gammaa*(sg/rhog)*(A_lag/Ag_lag)^phii);
% q=q+1;
% ceq(q) = An- An_lag*(1+gammaa*(sn/rhon)^etaa*(A_lag/An_lag)^phii);
% scientist demand
q=q+1;
ceq(q)= ws - (gammaa*(A_lag./Af_lag).^phii.*pf.*(1-tauf).*FF.*(1-alphaf).*Af_lag)./(rhof*Af); 
%8
q=q+1;
ceq(q)= ws - (gammaa*(A_lag./Ag_lag).^phii.*pg.*G.*(1-alphag).*Ag_lag)./(rhog*(1-taus)*Ag);
%9
q=q+1;
ceq(q)= ws - (gammaa*(A_lag./An_lag).^phii.*pn.*N.*(1-alphan).*An_lag)./(rhon.*An);
