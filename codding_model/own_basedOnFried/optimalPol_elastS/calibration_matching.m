...
sff = log(0.004);
sn  = log(0.003); %log((Sparams.S-Sparams.S*(1-1e-10))/(Sparams.S*(1-1e-10)));
...
%[c, ceq]=calibRem_nows(x0, MOM, list, trProd, parsHelp, polhelp, Af, An, Ag);
...
[x,fval,exitflag,output,lambda] = fmincon(objf,x0,[],[],[],[],lb,ub,handf,options);
