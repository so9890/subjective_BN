...
symms.optsym=[ mu_IMP mu_MarketS  mu_NProd mu_OPThh ... %KT_hh KT_hl KT_S ...
        mu_OPTLn mu_OPTLg mu_LOMAg mu_LOMAf mu_LOMAn...
        hhf hhg hlf hlg C F G Af Ag An HL HH S]; % dropped: mu_wageG
...
LOMAf   = Af-Af_lag.*(1+gammaa.*(sff./rhof).^etaa.*(A_lag./Af_lag).^phii);
LOMAn   = An-(An_lag.*(1+gammaa.*(sn./rhon).^etaa.*(A_lag./An_lag).^phii));
NProd   = N-An.*Ln.*(pn.*alphan).^(alphan./(1-alphan)); 
OPTLn  = hhn./hln-thetan/(1-thetan).*wl./wh; % optimality labour good producers green high
OPTLg  = hhg./hlg-thetag/(1-thetag).*wl./wh; % optimality labour good producers neutral low
OPThh   = chii*hh.^(sigmaa+taul)-(muu.*lambdaa.*(1-taul).*(wh).^(1-taul));
