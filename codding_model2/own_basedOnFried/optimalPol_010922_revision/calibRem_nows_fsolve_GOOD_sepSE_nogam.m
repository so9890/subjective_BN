function  [ceq]= calibRem_nows_fsolve_GOOD_sepSE_nogam(x, MOM, list, trProd, paramss, poll, Af, An, Ag, gammaa)
...
% scientist demand
q=q+1;
ceq(q)= wse - (gammaa*etaa*(A_lag./Af_lag).^phii.*sff.^(etaa-1).*pf.*FF.*(1-alphaf).*Af_lag)./(rhof^etaa.*Af); 
%8
q=q+1;
ceq(q)= wse - (gammaa*etaa*(A_lag./Ag_lag).^phii.*sg.^(etaa-1).*pg.*(1+taus)*G.*(1-alphag).*Ag_lag)./(rhog^etaa.*Ag);
%9
q=q+1;
...
% scientist supply 
q=q+1;
ceq(q)= se-(muu*wse/(chiis)).^(1/sigmaas);  % equal disutility as for other labour => pins down ws
q=q+1;
ceq(q)= sn-(muu*wsn/(chiis)).^(1/sigmaas);  % equal disutility as for other labour => pins down ws
% q=q+1;
% ceq(q)= sg-(muu*wsg/(chiis)).^(1/sigmaas);  % equal disutility as for other labour => pins down ws
