function  [ceq]= calibRem_nows_fsolve_GOOD_sep_nogam(x, MOM, list, trProd, paramss, poll, Af, An, Ag, gammaa)
...
% scientist demand
q=q+1;
ceq(q)= wsf - (gammaa*etaa*(A_lag./Af_lag).^phii.*sff.^(etaa-1).*pf.*FF.*(1-alphaf).*Af_lag)./(rhof^etaa.*Af); 
%8
q=q+1;
ceq(q)= wsg - (gammaa*etaa*(A_lag./Ag_lag).^phii.*sg.^(etaa-1).*pg.*(1+taus)*G.*(1-alphag).*Ag_lag)./(rhog^etaa.*Ag);
%9
q=q+1;
...
% scientist supply 
q=q+1;
ceq(q)= sff-(muu*wsf/(chiis)).^(1/sigmaas);  % equal disutility as for other labour => pins down ws
q=q+1;
